...
let studentName: string = "Akbarali";
let course: string = `${studentName} is taking a course on typescript`;
...
console.log(isStudent, isOnline);
...
let randomValue: any = "Typescript";
randomValue = 43;
randomValue = true;
...
let city = "25";
//city = 25 // Type 'number' is not assignable to type 'string'.ts(2322)
...
let carBrands: string[] = ["toyota", "bmw", "mercedes", "nissan"];
...
let scores: number[] = [23, 43, 12, 56, 86];
console.log(scores);
...
console.log(
  `${product.title} costs ${product.price} and its ${product.inStock}`,
);
